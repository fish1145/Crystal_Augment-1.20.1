package fish_dan.crystal_augment.fluid;
...
public class FluidCrystal_Augment extends FluidCoFH {

    private final String key;
    private final RegistryObject<FluidType> type;

    public FluidCrystal_Augment(String key, String stillTexture, String flowTexture, int temp) {
        super(Crystal_AugmentFluids.FLUIDS, key);

        this.key = key;

        type = Crystal_AugmentFluids.FLUID_TYPES.register(key, () -> new FluidType(FluidType.Properties.create()
                .lightLevel(12)
                .density(4000)
                .viscosity(2500)
                .temperature(temp)
        ...
        bucket = Crystal_AugmentItems.ITEMS.register(bucket(key), () -> new BucketItem(stillFluid, new Item.Properties().craftRemainder(Items.BUCKET).stacksTo(1)));
    }
